...
class SpellViewController: UITableViewController{
    var corrido = false
    var nombres = [String]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cellSpells")! as UITableViewCell
        cell.textLabel?.text = nombres[indexPath.row]
        return cell
    }
    
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        arrSpells.spell = nombres[indexPath.row]
        
    }
    
    func getSpells()
    {
        let sqlConsulta2 = "SELECT * FROM hechizos"
