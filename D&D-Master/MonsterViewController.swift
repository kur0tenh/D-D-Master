...
class MonsterViewController: UITableViewController{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cellMonstruos")! as UITableViewCell
        cell.textLabel?.text = nombres[indexPath.row]
        return cell
    }
    
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        arrMonstruos.monstruo = nombres[indexPath.row]
        
    }
    
    func getMonstruos()
    {
        let sqlConsulta2 = "SELECT * FROM monstruos"
